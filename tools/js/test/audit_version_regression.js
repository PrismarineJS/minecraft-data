...
function versionToComparableInteger (versionPath) {
  ...
  const [platform, version] = pathParts
  ...
  if (version.includes('w') && /^\d+w\d+[a-z]?$/.test(version)) {
    ...
    if (match) {
      ...
      if (year >= 2017 && year <= 2018) {
        // 17w* snapshots are for 1.13, 18w* are for 1.13.x
        approximateRelease = 1013000000 // 1.13.x base
        if (year === 2017 && week < 30) approximateRelease = 1012500000 // Between 1.12 and 1.13
      } else if (year >= 2019) {
        // 19w* and later snapshots - map roughly to version years
        const majorVersion = Math.max(14, year - 2005) // Rough approximation
        approximateRelease = 1000000000 + majorVersion * 1000000
      } else if (year <= 2016) {
        // Earlier snapshots
        approximateRelease = 1009000000 + year * 100000 + week * 1000
      }

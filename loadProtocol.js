...
function packetToString(state, direction, packet)
{
  ...
  function generateLines() {
    var packets = packet.fields.reduce(function (acc, field) {
      ...
      var type = fieldToLines(totalCols,field, 0, function(field, fieldInfo) {
...
function switchToLines(totalCols,field,fieldType,depth, getVal)
{
  ...
  var lines = Object.keys(elems).reduce(function(acc, key) {
    ...
    'if (' + eqs(fieldType.typeArgs.compareTo, elems[key]) + ')', "type": JSON.parse(key) }, depth + 1, getVal));
    firstLine = false;
    return acc;
  }, []);
...
function arrayToLines(totalCols,field,fieldType,depth, getVal)
{
  ...
  if (fieldType.typeArgs.countType)
    ...
    countLine = [];
  var contentLine = fieldToLines(totalCols,{
    name: '$content',
...
{
  $j.ajax("https://cdn.rawgit.com/"+repo+"/data/"+version+"/protocol.json")
    .done(function(data){
